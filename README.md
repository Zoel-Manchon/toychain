...
- **Tamper detection**: the `☠ tamper` button modifies a block's data without
  ...
  corruption cascade in real time.
...
`GET /api/v1/chain` (authenticated) returns the full chain plus its verdict:
...
Anyone with an operator session can re-verify the chain externally — the
validator's three checks are trivially reproducible from this payload.
...
- [ ] Per-block author (`mined_by`) — blocks signed by the operator who
      queued them
- [ ] API tokens for header-based auth (`Authorization: Bearer …`) so the
      JSON API works from curl/scripts without cookies
- [ ] External verifier script (Python) consuming `/api/v1/chain` and
