...
class BlocksControllerTest < ActionDispatch::IntegrationTest
  ...
  test "create enqueues a mining job" do
    ...
    assert_enqueued_with(job: MineBlockJob, args: [ "hello", 4, users(:zoel).id ]) do
  ...
  test "difficulty is clamped to the allowed range" do
    ...
    assert_enqueued_with(job: MineBlockJob, args: [ "sneaky", 6, users(:zoel).id ]) do
  ...
  test "missing difficulty falls back to the minimum" do
    ...
    assert_enqueued_with(job: MineBlockJob, args: [ "plain", 2, users(:zoel).id ]) do
